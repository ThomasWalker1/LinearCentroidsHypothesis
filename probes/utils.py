...
ACTS_ROOT = os.path.dirname('outputs/probes')
DATASET_ROOT = os.path.dirname('probes/datasets')
ACTS_BATCH_SIZE = 25
...
def collect_acts(dataset_name, model, layer, noperiod=False, center=True, scale=False, act_type='latents', device='cpu'):
    """
    Collects activations from a dataset of statements, returns as a tensor of shape [n_activations, activation_dimension].
    """
    directory = os.path.join(ACTS_ROOT, act_type)
    if noperiod:
    ...
    acts = [t.load(os.path.join(directory, f'layer_{layer}_{i}.pt'),weights_only=True).to(device) for i in range(0, ACTS_BATCH_SIZE * len(activation_files), ACTS_BATCH_SIZE)]
...
class DataManager:
    ...
    def add_dataset(self, dataset_name, model_size, layer, label='label', split=None, seed=None, noperiod=False, center=True, scale=False, device='cpu'):
        ...
        """
        acts = collect_acts(dataset_name, model_size, layer, noperiod=noperiod, center=center, scale=scale, act_type=self.act_type, device=device)
        df = pd.read_csv(os.path.join(DATASET_ROOT, 'datasets', f'{dataset_name}.csv'))
