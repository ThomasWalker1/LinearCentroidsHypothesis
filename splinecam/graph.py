...
def add_line_to_graph(G,
    ...
    v: new node vertices
    new_edges_list: list to record new edges
    '''
    ...
    edge = (start,node_names[0])
    G.add_edge(*edge,layer=layer_name,
               hyp=line_name
              )
    if new_edges_list is not None and layer_name > -1:
...
def create_poly_hyp_graph(poly, hyps, q=None, hyp_endpoints=None, dtype=torch.float64, verify=True, return_new_edges=False, current_layer_index=-1):

    new_edges_ids = [] if return_new_edges else None
    G = nx.Graph()
    ...
    for v_set,hyp_v1_v2_idx_set,node_names in zip(v_collect,hyp_v1_v2_idx_collect,node_names_collect):    
        ...
        add_line_to_graph(
            ...
            new_edges_list=new_edges_ids
        )
    
    uniq_hyp_idx = hyp_v1_v2_idx[::2,0]
    hyp_endpoint_nodes = new_node_idx.reshape(-1,2)

    
    if uniq_hyp_idx.shape[0] <= 1:
        
        for nodes,name in zip(hyp_endpoint_nodes.numpy(),uniq_hyp_idx):
            edge = (nodes[0],nodes[1])
            G.add_edge(
                *edge,layer=current_layer_index,hyp=name
            )
            if new_edges_ids is not None:
                new_edges_ids.append(edge)

        if return_new_edges:
            new_edges_coords = []
            for start_node, end_node in new_edges_ids:
                start_coords = G.nodes[start_node]['v']
                end_coords = G.nodes[end_node]['v']
                new_edges_coords.append(torch.stack([start_coords, end_coords]))
            return G, new_edges_coords
        return G, None
    ...
    if no_inter_idx.shape[0] != 0:
        ...
        if no_inter_idx.shape[0] == 1:
            edge = (hyp_endpoint_nodes.numpy()[no_inter_idx.cpu()][0],
    ...
    for ii,each_hyp in enumerate(uniq_hyp_idx):
        ...
        add_line_to_graph(
            ...
            v = verts,
            line_name = each_hyp,
            layer_name = current_layer_index,
            new_edges_list=new_edges_ids
        )
...
@torch.no_grad()
def to_next_layer_partition(cycles, Abw, current_layer, NN, dtype=torch.float64, device='cpu', return_new_edges=False):
    ...
    res_regions = []
    new_cyc_idx = []
    all_new_edges = [] if return_new_edges else None
    ...
    if return_new_edges:
        return res_regions, new_cyc_idx, all_new_edges
...
def _batched_gpu_op_2(method, data1, data2, batch_size, out_size, dtype=torch.float32, workers=2):
    ...
    for in_batch1,in_batch2 in dataloadr:
        ...
        out_batch = method(in_batch1,in_batch2)
...
def to_next_layer_partition_batched(cycles, Abw, current_layer, NN,
                                    ...
                                    batch_size=-1, fwd_batch_size=-1, workers=2, return_new_edges=False):
    
    if batch_size == -1:
        return to_next_layer_partition(
            cycles, Abw, current_layer, NN, dtype, device, return_new_edges
        )
