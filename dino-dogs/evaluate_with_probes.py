...
def get_dataloaders(
    ...
    """Loads pre-computed features, remaps labels to 0-N, and creates DataLoaders."""
    train_path = base_data_path / f"train-{token_type}"
    test_path = base_data_path / f"test-{token_type}"
    ...
    # Find unique labels and sort them to ensure consistent 0-9 mapping
    unique_labels = torch.unique(train_labels)
    sorted_unique_labels = torch.sort(unique_labels)[0]
    
    # Create empty tensors to hold the new 0-9 mapped labels
    remapped_train_labels = torch.zeros_like(train_labels)
    remapped_test_labels = torch.zeros_like(test_labels)
    
    # Vectorized mapping: replace old ImageNet indices with new 0-9 indices
    for new_idx, old_idx in enumerate(sorted_unique_labels):
        remapped_train_labels[train_labels == old_idx] = new_idx
        remapped_test_labels[test_labels == old_idx] = new_idx
        
    # Replace the old label tensors with the new mapped ones
    train_labels = remapped_train_labels
    test_labels = remapped_test_labels
    # -----------------------------
    ...
    input_dim = train_inputs.size(1)
    # This will now safely be exactly 10
...
def main():
    ...
    parser.add_argument("--output_dir", type=Path, default=Path("outputs/dino-dogs"), help="Directory to save plots and results.")
    ...
    for config in tqdm(run_configs, desc="Evaluating Runs"):
        ...
        autoencoder.load_state_dict(torch.load(model_path, map_location=args.device, weights_only=False))
