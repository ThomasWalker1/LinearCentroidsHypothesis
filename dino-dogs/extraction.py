...
def get_dataloaders() -> Dict[str, DataLoader]:
    """
    Prepares and returns DataLoaders for a reproducible 10-class random subset 
    of ImageNet dog classes for training and validation sets.
    """
    print("Loading full ImageNet dataset...")
    
    # Initialize full datasets (Note: download=True is not supported for ImageNet)
    train_dataset = torchvision.datasets.ImageNet(
        DATA_ROOT, split="train", transform=DINO_TRANSFORM
    )
    test_dataset = torchvision.datasets.ImageNet(
        DATA_ROOT, split="val", transform=DINO_TRANSFORM
    )
    ...
    selected_class_names = [train_dataset.classes[i] for i in selected_classes]
    
    print(f"Selected 10 random dog class indices: {selected_classes}")
    print(f"Selected dog breeds: {selected_class_names}")
    
    # Find all sample indices that belong to the selected classes
    print("Subsetting datasets...")
    train_indices = [
        i for i, target in enumerate(train_dataset.targets) if target in selected_classes
    ]
    test_indices = [
        i for i, target in enumerate(test_dataset.targets) if target in selected_classes
    ]
    
    # Create PyTorch Subsets using the filtered indices
...
def main():
    ...
    (SAVE_DIR / "v2").mkdir(parents=True, exist_ok=True)
    print(f"Using device: {DEVICE}")

    loaders = get_dataloaders()
    model, block = setup_model_and_block("v2","facebook/dinov2-base")

    for set_type, loader in loaders.items():
        extract_and_save_features(model, "v2", block, loader, set_type, "cls")
